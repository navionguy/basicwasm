...
func Test_ReadProg(t *testing.T) {
	tests := []struct {
		...
		{inp: []byte{0x7C, 0x12, 0x0A, 0x00, 0x91, 0x20, 0x22, 0x48, 0x65, 0x6C,
			...
			0x20, 0x0F, 0x30, 0x00, 0x00, 0x00, 0x1A}, srcLines: 3},
	}
	...
	for _, tt := range tests {
		...
		env := object.NewTermEnvironment(mt)
		rdr.readProg(env)
		assert.EqualValues(t, tt.srcLines, env.GetSrcLineCount())
	}
}
...
func Test_RemStatementShortForm(t *testing.T) {
	tests := []struct {
		...
		{inp: []byte{0x7C, 0x12, 0x0A, 0x00, 0x3A, 0x8F, 0xD9, 0x20, 0x65, 0x6C,
			0x6C, 0x6F, 0x22, 0x00, 0x87, 0x12, 0x14, 0x00, 0x59, 0x20, 0xE7,
			0x20, 0x0F, 0x96, 0x00, 0x92, 0x12, 0x1E, 0x00, 0x5A, 0x20, 0xE7,
			0x20, 0x0F, 0x30, 0x00, 0x00, 0x00, 0x1A}, srcLines: 1},
	}
	...
	for _, tt := range tests {
		...
		rdr.readProg(env)
		lc := env.GetSrcLineCount()

		assert.Equal(t, tt.srcLines, lc)
	}
...
func Test_ReadProtProg(t *testing.T) {
	tests := []struct {
		...
		// ToDo: Update test so they are not both line 10
		{inp: []byte{0xCD, 0xA9, 0xBF, 0x54, 0xE2, 0x12, 0xBD, 0x59, 0x20, 0x65, 0x0D, 0x8F, 0xA2, 0x30, 0x98, 0xD3, 0x3E, 0xD3, 0xF1, 0xE6, 0x13, 0xA4}, lines: 1},
		{inp: []byte{0xCB, 0xA9, 0xBF, 0x54, 0xE2, 0x12, 0xBD, 0x59, 0x1C, 0x18, 0x7B, 0x02, 0xC8, 0x87, 0x78, 0xC5, 0x19, 0xCF, 0x94, 0x74, 0x87, 0xA4, 0x6C, 0x03}, lines: 1},
	}
...
func Test_ParseProtProg(t *testing.T) {
	tests := []struct {
		...
		// ToDo: Update tests so they all aren't line 10
		{inp: []byte{0xFE, 0xE3, 0xA9, 0xBF, 0x54, 0xE2, 0x12, 0xBD, 0x59, 0x1C, 0x18, 0x7B, 0x02, 0xC8}, srclines: 1}, // test case colon at eol
		{inp: []byte{0xFE, 0xE3, 0xA9, 0xBF, 0x54, 0xE2, 0x12, 0xBD}, srclines: 1},                                     // test case dblquote at eol
		{inp: []byte{}, srclines: 1},
		{inp: []byte{0xff}, srclines: 1},
		{inp: []byte{0xfe, 0xCD, 0xA9, 0xBF, 0x54, 0xE2, 0x12, 0xBD, 0x59, 0x20, 0x65, 0x0D, 0x8F, 0xA2, 0x30, 0x98, 0xD3, 0x3E, 0xD3, 0xF1, 0xE6, 0x13, 0xA4}, srclines: 1},
	}
